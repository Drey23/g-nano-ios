...
class HybridModerationService {
    
    // Instantiate both of the services we built earlier
    private let nativeService = NativeAppleModerationService()
    private let cloudService = CloudModerationService()
    ...
    func moderateImage(uiImage: UIImage) async -> ModerationResult {
        ...
        if #available(iOS 18.1, *), SystemLanguageModel.default.isAvailable {
            ...
            let result = await nativeService.moderateImageOffline(uiImage: uiImage)
            
            // Optional: If the native service fails for an unexpected reason,
            // you can still fall back to the cloud as a safety net!
            if result.reason.contains("Native processing failed") {
                print("⚠️ Native failed. Bouncing to Gemini Cloud.")
                return await cloudService.moderateImage(uiImage: uiImage)
            }
